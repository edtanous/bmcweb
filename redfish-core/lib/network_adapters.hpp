...
inline void
    getNetworkAdapterCollectionMembers(std::shared_ptr<bmcweb::AsyncResp> aResp,
    ...
    crow::connections::systemBus->async_method_call(
        [collectionPath, isNDF, aResp{std::move(aResp)}](
            ...
            const dbus::utility::MapperGetSubTreePathsResponse& objects) {
                // currently host name is hard coded. We will add support for multiple hosts through 
    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    crow::connections::systemBus->async_method_call(
        [chassisId, asyncResp](
            ...
            {
                nlohmann::json::object_t member;
                member["@odata.id"] = crow::utility::urlFromPieces("redfish", "v1", "Chassis", chassisId,
        ...
        "xyz.openbmc_project.ObjectMapper",
        "/xyz/openbmc_project/object_mapper",
        "xyz.openbmc_project.ObjectMapper", "GetSubTreePaths", "/xyz/openbmc_project/network/", 0, 
        std::array<std::string, 1>{"xyz.openbmc_project.Network.EthernetInterface"});
}
    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    // Support for reading the values from backend will be done through https://redmine.mellanox.com/issues/3461424
    ...
    asyncResp->res.jsonValue["@odata.id"] = crow::utility::urlFromPieces(
        "redfish", "v1", "Chassis", chassisId, "NetworkAdapters", PLATFORMNETWORKADAPTER);
    asyncResp->res.jsonValue["Ports"]["@odata.id"] =
        crow::utility::urlFromPieces("redfish", "v1", "Chassis", chassisId,
                                     "NetworkAdapters", PLATFORMNETWORKADAPTER, "Ports");
    ...
    const std::optional<std::string>& validChassisPath)
{
    ...
    if (isPort)
    {
        ...
        asyncResp->res.jsonValue["@odata.id"] = crow::utility::urlFromPieces(
            "redfish", "v1", "Chassis", chassisId, "NetworkAdapters", PLATFORMNETWORKADAPTER, "Ports");
        isNDF = false;
    }
    else
    {
        ...
        asyncResp->res.jsonValue["@odata.id"] = crow::utility::urlFromPieces(
            "redfish", "v1", "Chassis", chassisId, "NetworkAdapters", PLATFORMNETWORKADAPTER, "NetworkDeviceFunctions");
    }
    getNetworkAdapterCollectionMembers(
        asyncResp, "/redfish/v1/Chassis/" + chassisId + "/NetworkAdapters/" PLATFORMNETWORKADAPTER  + collectionName, isNDF,
        {"xyz.openbmc_project.Network.EthernetInterface"}, "/xyz/openbmc_project/network/");
}
    ...
    const std::string& param)
{
    ...
    redfish::chassis_utils::getValidChassisPath(
        asyncResp, chassisId,
        std::bind_front(doPortNDFCollection, asyncResp,
                        chassisId, false));
}
                           ...
                           const std::string& portId)
{
    ...
    asyncResp->res.jsonValue["LinkNetworkTechnology"] =
        "Ethernet";
    asyncResp->res.jsonValue["@odata.id"] = crow::utility::urlFromPieces(
        "redfish", "v1", "Chassis", chassisId, "NetworkAdapters", PLATFORMNETWORKADAPTER, "Ports", portId);
                           ...
                           const std::string& portId)
{
    ...
    links["PhysicalPortAssignment"]["@odata.id"] = "/redfish/v1/Chassis/"
            + chassisId + "/NetworkAdapters/" PLATFORMNETWORKADAPTER "/Ports/" + portId;
    links["OffloadSystem"]["@odata.id"] = "/redfish/v1/Systems/" PLATFORMSYSTEMID;
    asyncResp->res.jsonValue["Name"] = "NetworkDeviceFunction";
    asyncResp->res.jsonValue["NetDevFuncType"] = "Ethernet";
    asyncResp->res.jsonValue["NetDevFuncCapabilities"] = nlohmann::json::array({"Ethernet"});
    asyncResp->res.jsonValue["@odata.id"] = "/redfish/v1/Chassis/"
            + chassisId + "/NetworkAdapters/" PLATFORMNETWORKADAPTER "/NetworkDeviceFunctions/" + ndfId;
              ...
              bool isNDF = true)
{
    ...
    crow::connections::systemBus->async_method_call(
        [asyncResp, interfaces, chassisId, id, isNDF](
            const boost::system::error_code ec,
                        ...
                        object : subtree)
            {
                ...
                {
                    doNDF(asyncResp, path, connectionName, chassisId, id, objPath.filename());
                }
                else
                {
            ...
            messages::resourceNotFound(
                asyncResp->res, "#NetworkDeviceFunction.v1_9_0.NetworkDeviceFunction", chassisId);
        },
        "xyz.openbmc_project.ObjectMapper",
        "/xyz/openbmc_project/object_mapper",
...
inline void requestRoutesNetworkAdapters(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/NetworkAdapters/")
        ...
        .methods(boost::beast::http::verb::get)(std::bind_front(
            handleNetworkAdaptersCollectionGet, std::ref(app)));
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/NetworkAdapters/" PLATFORMNETWORKADAPTER)
        .privileges(redfish::privileges::getNetworkAdapter)
        .methods(boost::beast::http::verb::get)(std::bind_front(
            handleNetworkAdapterGet, std::ref(app)));
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/NetworkAdapters/" PLATFORMNETWORKADAPTER "/NetworkDeviceFunctions/")
        .privileges(redfish::privileges::getNetworkDeviceFunctionCollection)
        .methods(boost::beast::http::verb::get)(std::bind_front(
            handleNetworkDeviceFunctionsCollectionGet, std::ref(app)));
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/NetworkAdapters/" PLATFORMNETWORKADAPTER "/Ports")
        .privileges(redfish::privileges::getPortCollection)
        .methods(boost::beast::http::verb::get)(std::bind_front(
            handlePortsCollectionGet, std::ref(app)));
}

inline void requestRoutesNetworkDeviceFunctions(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/NetworkAdapters/" PLATFORMNETWORKADAPTER "/NetworkDeviceFunctions/<str>/")
        .privileges(redfish::privileges::getNetworkDeviceFunction)
        .methods(boost::beast::http::verb::get)(std::bind_front(
            handleNDFGet, std::ref(app)));
}

inline void requestRoutesACDPort(App& app)
{
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/NetworkAdapters/" PLATFORMNETWORKADAPTER "/Ports/<str>/")
        .privileges(redfish::privileges::getPort)
        .methods(boost::beast::http::verb::get)(std::bind_front(
            handlePortGet, std::ref(app)));
}
#endif
