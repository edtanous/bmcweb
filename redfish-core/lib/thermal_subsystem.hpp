...
inline void doThermalSubsystemFansCollection(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& chassisId,
    const std::optional<std::string>& validChassisPath)
{
    ...
    auto sensorAsyncResp = std::make_shared<SensorsAsyncResp>(
        asyncResp, chassisId, sensors::dbus::thermalPaths,
...
inline void handleThermalSubsystemFansCollectionGet(
    App& app, const crow::Request& req,
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const std::string& param)
{
    ...
    redfish::chassis_utils::getValidChassisPath(
        asyncResp, chassisId,
        std::bind_front(doThermalSubsystemFansCollection, asyncResp,
                        chassisId));
}
...
inline void requestRoutesThermalSubsystem(App& app)
{
    ...
    BMCWEB_ROUTE(app, "/redfish/v1/Chassis/<str>/ThermalSubsystem/Fans")
        .privileges(redfish::privileges::getThermalSubsystem)
        .methods(boost::beast::http::verb::get)(std::bind_front(
            handleThermalSubsystemFansCollectionGet, std::ref(app)));
}
