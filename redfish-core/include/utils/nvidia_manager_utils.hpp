...
namespace nvidia_manager_util
{
    ...
    */
        ...
        [aResp](const boost::system::error_code ec,
                     ...
                     property : propertiesList)
            {
                if (property.first == "FeatureType")
                {
                    ...
                    if (*value == "xyz.openbmc_project.State.FeatureReady.FeatureTypes.Manager")
                    {
                        for (const std::pair<std::string, std::variant<std::string>>&
    ...
    inline void getOemReadyState(const std::shared_ptr<bmcweb::AsyncResp>& asyncResp, const std::string &bmcId)
    {
                                ...
                                object : subtree)
                    {
                        ...
                        for (const auto& interfaceName : interfaces)
                        {
                            if (interfaceName ==
                                ...
                                "FeatureReady")
                            {
                                getOemManagerState(asyncResp, connectionName,
                                                path);
                            }
                        }
                    }
