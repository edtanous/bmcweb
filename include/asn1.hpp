 ...
 */
template <typename T>
inline const T* safePtrArithmeticConst(std::span<const T> safeBuf, size_t offset)
{
    ...
    bool *pbkdf)
{
    ...
    if (d2i_ASN1_OBJECT(&o, &buf, static_cast<long>(length)) != nullptr) 
    {
        ...
        if (std::equal(std::begin(pbesid), std::end(pbesid), std::begin(membuf)))
        {
            *pbes = true;
        }
        else if (std::equal(std::begin(pbkdfid), std::end(pbkdfid), std::begin(membuf)))
        {
            *pbkdf = true;
        }
...
inline int hasPbesPbkdf(
    ...
    bool *pbkdf)
{
    const unsigned char *p = nullptr, *ep = nullptr, *tot = nullptr, *op = nullptr;
    long len = 0;
    size_t hl = 0;
    int r = 0;
    int ret = 0;
    ...
    while (length > 0) {
        op = p;
        asn1ObjectType = ASN1_get_object(&p, &len, &asn1ObjectTag, &asn1ObjectClass, static_cast<long>(length));
        ...
        if (asn1ObjectType & V_ASN1_CONSTRUCTED) {
            ...
            // ep = p + len;
            ep = safePtrArithmeticConst<unsigned char>( { p, (length+1) }, static_cast<size_t>(len));
        ...
        else 
        {
            ...
            {
                ret = checkAsn1Object(op, (static_cast<size_t>(len)+hl), pbes, pbkdf);
                if (ret == 2137)
                {
                    break;
                }
            ...
            // p += len;
            p = safePtrArithmeticConst<unsigned char>( { p, (length+1) }, static_cast<size_t>(len));
...
inline int pemPkeyIsEncrypted(const std::string &filename, bool *encrypted)
{
    ...
    {
        BMCWEB_LOG_ERROR << "Error while processing ASN.1 structures at " << filename 
            << " return value: " << ret << "\n";
        BIO_free(inB64);
        BUF_MEM_free(buf);
        BIO_free(in);
