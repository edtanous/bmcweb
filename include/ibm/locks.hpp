...
inline bool Lock::isValidLockRequest(const LockRequest& refLockRecord)
{
    ...
    if (!((boost::equals(refLockRecord.locktype, "Read") ||
           (boost::equals(refLockRecord.locktype, "Write")))))
    {
                                   ...
                                   const LockRequest& refLockRecord2)
{
    ...
    if (boost::equals(refLockRecord1.locktype, "Read") &&
        boost::equals(refLockRecord2.locktype, "Read"))
    {
    ...
    for (const auto& p : refLockRecord1.segmentinfo)
    {
        ...
        if (boost::equals(p.first, "LockAll") ||
            boost::equals(refLockRecord2.segmentinfo[i].first, "LockAll"))
        {
            BMCWEB_LOG_DEBUG(
        ...
        if ((boost::equals(p.first, "LockSame") ||
             boost::equals(refLockRecord2.segmentinfo[i].first, "LockSame")) &&
