...
public:
    ...
    void watch(
        std::function<void(std::vector<FileWatcherEvent>)> handler
    ){
        if (sd == nullptr) {
            return;
        }
        ...
        sd->async_read_some(
            ...
            boost::bind(
                &InotifyFileWatcher::asyncReadHandler, 
                this, 
...
private:
    ...
    void asyncReadHandler(
        ...
        while (offset < bytes) 
        {
            ...
            std::memcpy(
                alignedEvp, (buf.data() + offset), 
                sizeof(inotify_event) + NAME_MAX + 1 - offset);

            FileWatcherEvent fwev(
                std::string(dirForEvent(*alignedEvp)), 
