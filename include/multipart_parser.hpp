...
struct FormPart
{
    ...
    std::variant<std::string, TemporaryFileHandle> content;
    bool isFile() const
    {
        return std::holds_alternative<TemporaryFileHandle>(content);
    }
};
...
class MultipartParser
{
        ...
        for (size_t i = 0; i < len; ++i)
        {
            ...
            switch (state)
            {
                ...
                case State::HEADERS_ALMOST_DONE:
                {
                    ...
                    {
                        mime_fields.back()
                            .content.emplace<TemporaryFileHandle>();
                    }
  ...
  private:
    ...
    ParserError writeData(std::string_view buffer)
    {
        ...
        if (auto* file = std::get_if<TemporaryFileHandle>(&content))
        {
            boost::system::error_code ec;
            file->fileHandle.fileHandle.write(buffer.data(), buffer.size(), ec);
            if (ec)
            {
