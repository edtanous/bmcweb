...
void sendOtel(crow::Request& request)
{
    ...
    namespace SemanticConventions = opentelemetry::trace::SemanticConventions;
    ...
    std::vector<
        ...
        attributes = {
            ...
            {SemanticConventions::kHttpRequestMethod, request.methodString()}};
...
class BmcwebSpanExporter final : public opentelemetry::sdk::trace::SpanExporter
{
  ...
  private:
    static nlohmann::json::object_t encodeValueToJson(auto&& value)
    {
        ...
        else if constexpr (bmcweb::IsVector<BaseType>::value)
        {
    ...
    nlohmann::json::object_t
        SpanToJson(opentelemetry::sdk::trace::SpanData& span)
    {
        ...
        spanJson["parentId"] = std::string_view(parent_span_id.data(),
                                                parent_span_id.size());
        ...
        for (const auto& linkKV : span.GetLinks())
        {
            ...
            linkJson["traceId"] = std::string_view(link_trace_id.data(),
                                                   link_trace_id.size());

            std::array<char, 16> link_span_id{};
            linkKV.GetSpanContext().span_id().ToLowerBase16(link_span_id);
            linkJson["spanId"] = std::string_view(link_span_id.data(),
                                                  link_span_id.size());
            linkJson["traceState"] =
    ...
    nlohmann::json::object_t
        ToJson(std::span<std::unique_ptr<opentelemetry::sdk::trace::Recordable>>
                   records)
    {
